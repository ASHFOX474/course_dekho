-- course-dekho:migration 0012
-- Forward-only: allow admin-authored revisions to reclassify existing content.
-- Preserves pending-first review and immutable submissions/revisions; no data rewrite.
...
BEGIN
    ...
    IF NEW.target_content_id IS NOT NULL THEN
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'Target content not found' USING ERRCODE = '23514';
        END IF;
        IF target_type IS DISTINCT FROM NEW.resource_type AND NOT EXISTS (
            SELECT 1 FROM coursedekho.app_user WHERE id = NEW.submitted_by_user_id AND role = 'admin' AND is_active
        ) THEN
