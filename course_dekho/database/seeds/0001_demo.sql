...
-- Demo accounts are pre-approved (registration_status = 'approved') since
-- they exist to be logged into immediately for grading/demo purposes, not
-- to exercise the new-registration approval queue.
INSERT INTO coursedekho.app_user (
    public_id, name, email, username, password_hash, role,
    registration_status, reviewed_at
)
VALUES
    (
        ...
        'learner'::coursedekho.user_role,
        'approved'::coursedekho.registration_status,
        now()
    ),
    (
        ...
        'contributor'::coursedekho.user_role,
        'approved'::coursedekho.registration_status,
        now()
    ),
    (
...
SET
    ...
    role = EXCLUDED.role,
    registration_status = 'approved'::coursedekho.registration_status,
    reviewed_at = now(),
