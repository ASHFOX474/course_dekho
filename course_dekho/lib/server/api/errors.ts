...
export class ConflictError extends ApplicationError {
  constructor(message = "The request conflicts with current state.", fieldErrors?: Record<string, string[]>) {
    super("CONFLICT", 409, message, fieldErrors);
...
// Credentials were correct, but the account has not cleared admin review yet.
// Distinct from UnauthenticatedError (wrong/unknown credentials) so the client
// can show "your account is awaiting approval" instead of a generic auth failure.
export class AccountPendingApprovalError extends ApplicationError {
  constructor(message = "This account is awaiting admin approval.") {
    super("ACCOUNT_PENDING_APPROVAL", 403, message);
    this.name = "AccountPendingApprovalError";
  }
}

export class AccountRejectedError extends ApplicationError {
  constructor(message = "This account's registration was not approved.") {
    super("ACCOUNT_REJECTED", 403, message);
