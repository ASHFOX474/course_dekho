...
export function validateRejectUserRequest(value: unknown): RejectUserRequestDto {
  return validateRejectSubmissionRequest(value);
}
...
export function validateRegisterRequest(value: unknown): RegisterRequestDto {
  ...
  if (role !== "learner" && role !== "contributor") {
    addError(errors, "role", "Public registration supports learner or contributor accounts only.");
  }
  ...
  if (role === "contributor" && value.yearOfStudy !== undefined) {
    addError(errors, "yearOfStudy", "yearOfStudy is only valid for learner accounts.");
  }
  ...
  if (role === "learner" && yearOfStudy !== undefined) result.yearOfStudy = yearOfStudy;
  return result;
}
