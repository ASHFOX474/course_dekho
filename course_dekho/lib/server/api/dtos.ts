...
// A learner/contributor account awaiting admin review. Kept separate from
// UserSummaryDto since it exposes fields (registeredAt, universityName) that
// are only meaningful in the review queue, not in a normal session payload.
...
export type RejectUserRequestDto = RejectSubmissionRequestDto;

// The admin's full user directory: every account, any role, any review
// state. Distinct from PendingUserDto (pending-only queue).
