...
const catalogRoles: readonly UserRole[] = ["learner", "contributor", "admin"];
...
export function createCatalogHttpHandlers(dependencies: CatalogHttpDependencies) {
  return {
    // Deliberately UNauthenticated: the sign-up form needs to populate a university
    // picker before the person has an account or a session to send. Which universities
    // exist is not sensitive data, unlike everything else in this file (course/topic/
    // resource browsing), which stays gated to logged-in users only via
    // authorizeCatalogRead. Do not reuse this handler for anything else.
    async listPublicUniversities(): Promise<Response> {
      try {
