...
// Unauthenticated counterpart to listUniversities, used only by the sign-up form
// (app/login/page.tsx) before a session exists. Hits /api/v1/public/universities,
// not /api/v1/universities — the latter requires a logged-in session and will 401
// during sign-up.
export function listPublicUniversities(signal?: AbortSignal): Promise<UniversitySummaryDto[]> {
  return getData("/api/v1/public/universities", signal);
}
